...
2. **Plan second** — Use `/feature-plan` to define HOW and WHERE (includes a Test Matrix in full mode)
...
4. **Implement with TDD** — For each plan task, use `/tdd` (which consumes the test plan or Test Matrix) to write tests first, then implement. Pure functions get tests even if the task is marked `Tests: No`.
5. **Review** — Review changes before committing
6. **Commit** — Create atomic, well-described commits

Skip to step 4 for trivial changes (config tweaks, copy edits, single-file fixes).

## Parallel Execution

During step 4, tasks that have no dependency conflicts (`Depends on: None` or all dependencies already completed) SHOULD be executed in parallel via subagents. The implementation agent should:
